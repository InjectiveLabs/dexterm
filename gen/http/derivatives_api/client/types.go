...
func ValidateDerivativeOrderResponseBody(body *DerivativeOrderResponseBody) (err error) {
	...
	if body.MakerFeeAssetData != nil {
		err = goa.MergeErrors(err, goa.ValidatePattern("body.makerFeeAssetData", *body.MakerFeeAssetData, "^0x(([0-9a-fA-F][0-9a-fA-F])+)?$"))
	}
	if body.TakerFeeAssetData != nil {
		err = goa.MergeErrors(err, goa.ValidatePattern("body.takerFeeAssetData", *body.TakerFeeAssetData, "^0x(([0-9a-fA-F][0-9a-fA-F])+)?$"))
	}
	if body.Signature != nil {
